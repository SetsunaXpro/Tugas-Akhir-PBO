    ...
    Piece.prototype = {
        ...
        hvd_legal : function(){
            ...
            $.each(this.footprint, function(i){
                ...
                const rawMoves = Game.vector(self.position, this, self.color, true).list.split(',');
                const cleanMoves = rawMoves.map(m => m.replace(/^#/, ''));
                moves = legal.moves.concat(cleanMoves);
         ...
         */
        Legal : function() {
             // Ensure _legal() returns the proper structure
    var legal = this._legal();
    
    // Safeguard against undefined values
    if (!legal) {
        console.warn("_legal() returned undefined for", this);
        legal = { moves: [], capture: [] };
    }
    
    if (!legal.moves) legal.moves = [];
    if (!legal.capture) legal.capture = [];
    
    // Create comma-separated list only if moves exist
    legal.moveList = legal.moves ? legal.moves.join(',') : '';
...
rook.prototype._legal = function() {
    console.log('--- ROOK DEBUG ---');
    console.log('Current position:', this.position);
    
    const moves = [];
    const capture = [];
    const [col, row] = [this.position[0], this.position[1]];
    
    // Horizontal moves (same row)
    console.log('Checking horizontal moves:');
    for (let c = 0; c < 8; c++) {
        const newCol = Game.cLabels[c];
        if (newCol !== col) {
            const square = newCol + row;
            const piece = Game.occupied(square);
            console.log('  ', square, ':', piece ? piece.type : 'empty');
    ...
    console.log('Checking vertical moves:');
    for (let r = 1; r <= 8; r++) {
        if (r != row) {
            const square = col + r;
            const piece = Game.occupied(square);
    ...
    console.log('Final captures:', capture.map(p => p.type));
...
if (kid && kid.color == Game.Players[0].color) {
    ...
    const legal = kid.Legal();

    // Highlight movement squares
    legal.moves.forEach(function(squareId) {
        $('#' + squareId.replace(/^#/, '')).addClass('legal');
    });

    // Highlight capturable enemy pieces
    legal.capture.forEach(function(piece) {
        $('#' + piece.position.replace(/^#/, '')).addClass('legal');
    });
}
